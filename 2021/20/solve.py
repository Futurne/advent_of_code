...
def draw_image(image: np.ndarray):
    index_shape = np.array(['.', '#'])
    image = index_shape[image]
    for row in image:
        print(*row, sep='')
...
def read_input(path: str) -> tuple:
    with open(path, 'r') as input_file:
        ...
        enhancer_alg = lines[0]
        image = lines[2:]


    enhancer_alg = np.array(list(enhancer_alg)) == '#'
    image = [
        list(row)
        for row in image
    ]
    ...
    return enhancer_alg.astype(int), image.astype(int)


def enhance_step(image: np.ndarray, enhancer: np.ndarray, fillvalue: int) -> np.ndarray:
    image = convolve2d(image, CONV, mode='full', fillvalue=fillvalue)
    return enhancer[image]


def main(input_path: str, n_loops: int) -> int:
    enhancer, image = read_input(input_path)

    fillvalue = 0
    for i in range(n_loops):
        image = enhance_step(image, enhancer, fillvalue)
        fillvalue = enhancer[-fillvalue]

    return np.sum(image)


if __name__ == '__main__':
    print('Solution for day 20')

    solution = main('input', 2)
    print('Part 1:', solution)

    solution = main('input', 50)
